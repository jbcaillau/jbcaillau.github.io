...
\exo{ Soient $f\in L^1(\R,\lambda)$ et $g\in L^p(\R,\lambda)$ avec $1\le p\le +\infty$ et $\lambda$ la mesure de Lebesgue. Montrer que pour presque tout $x\in \R$, la fonction $y\mapsto f(x-y)g(y)$ est intégrable sur $\R$ et que le produit de convolution de $f$ et $g$ défini par
$$
f\star g(x)=\int_{\R} f(x-y)g(y)\,dy
$$
vérifie $f\star g(x)=g\star f(x)$ et 
$$
...
\em{Indication : on pourra utiliser l'exercice 6.}
\end{enumerate}
}

\exo{
  \begin{enumerate}
    \item Soient $f\in L^1(\R^d,\lambda)$ et $g\in L^p(\R^d,\lambda)$ avec $p\in [1,\infty[$. Monter que 
    \begin{equation*}
